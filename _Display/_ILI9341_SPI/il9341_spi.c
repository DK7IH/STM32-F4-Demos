...
///////////////////////////////////////////////////////////////////
//
//This "bare metal" demo shows how to set up and use
...
//MOSI(SDI) = PB15
//
//PB0:   ADC in for V.batt
//PB1:   ADC in for V.in
//PC150: ADC in for I.charge
...
void lcd_draw_line_h(int, int, int, int);
void lcd_draw_line_v(int, int, int, int);
...
uint16_t lcd_read(uint8_t reg)
{
	//Send reg data
    SPI2->DR = reg;
    //Wait until TX buf is empty (TXE flag)
    while(!(SPI2->SR & (1 << 1)));
    //Wait until RX buf is filled (RXNE flag)
    while(!(SPI2->SR & (1 << 0)));
...
void lcd_draw_line_h(int x0, int x1, int y0, int color)
{
...
void lcd_draw_line_v(int y0, int y1, int x0, int color)
{
...
int main(void)
{
    ...
    lcd_cls(bcolor);
    //Frame
    lcd_draw_line_h(0, LCD_WIDTH, 0, WHITE);
    lcd_draw_line_h(0, LCD_WIDTH, LCD_HEIGHT, WHITE);
    lcd_draw_line_v(0, LCD_HEIGHT, 0, WHITE);
    lcd_draw_line_v(0, LCD_HEIGHT, LCD_WIDTH, WHITE);
    
    //Msg Line
    lcd_draw_line_h(0, LCD_WIDTH, FONTHEIGHT + 4, WHITE);
